...
function tree_force(node, root) {
	if (root['val']) {
		...
		force = vector_add(force, centerPull(node));
...
function cool() {
	if (damping - .00005 > 0) {damping -= .00005};
}
...
function tick() {
	...
	for (var i=0, l=nodes.length; i < l; i++) {
		...
		vel[0] = (vel[0] + (timeStep * force[0]/mass)) * damping;
		vel[1] = (vel[1] + (timeStep * force[1]/mass)) * damping;
	...
	if (lowEntropy < 30 && maxVel > 0) {
		var wait = (interval * 1000) - ((new Date()).getTime() - start);
		if (wait < 0) {wait = 0;}
